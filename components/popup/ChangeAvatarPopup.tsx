...
import { updateBoardMember } from '../../firebase/store/boardMember'
  ...
  const handleChange = () => {
    const processCommands = async () => {
      if (hasAvatarChanged && selectedAvatar) {
        ...
        await updateBoardMember(user!.email, {
          avatarUrl: selectedAvatar,
        })
        await revalidateUrls([getMemberPageUrl(user!.username)])
        methods.updateBoardMember(updatedUserInfo)
